...
class SearchNode:
    ...
    def visit(self, eval_func, target_moves):
        ...
        if self.visits == 0:
            self.visits = 1
            policy, moves, r50_est, sorted_high_policy, sorted_indicies = eval_func(
                self.input_data)
        ...
        if best_child == len(self.children):
            ...
            try:
                updateBoardForIndex(new_child.board, new_child.state,
                                    self.policy_index[best_child], self.flip)
                ...
                if new_child.board.board_fen(
                ) == "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR" and new_child.board.turn == chess.WHITE:
                    new_child.good = True
                    new_child.visits = 1
                    new_child.total_move_est = utility_calc(
                        0, target_moves - 1)
            self.children.append(new_child)
...
def updateBoardForIndex(board, state, max_idx, flip):
    ...
    if move_type < 48:
        ...
        if cap_type == 1:
            board.set_piece_at(
                sq,
                chess.Piece(chess.QUEEN, chess.BLACK if flip else chess.WHITE))
        elif cap_type == 2:
            board.set_piece_at(
                sq,
                chess.Piece(chess.ROOK, chess.BLACK if flip else chess.WHITE))
        elif cap_type == 3:
            board.set_piece_at(
                sq,
                chess.Piece(chess.BISHOP,
        ...
        if piece_moved.piece_type == chess.ROOK and direction not in [
                0, 2, 4, 6
        ]:
            raise ValueError("Rook doing diagonal moves.")
        if piece_moved.piece_type == chess.BISHOP and direction not in [
                1, 3, 5, 7
        ]:
            raise ValueError("Bishop doing horiz/vert moves.")
        if piece_moved.piece_type == chess.KING and (abs(to_row - row) > 1
                                                     or abs(to_col - col) > 1):
        ...
        if piece_moved.piece_type == chess.PAWN and (
                abs(to_row - row) + abs(to_col - col) > 2):
        ...
        board.set_piece_at(
            from_sq,
            chess.Piece(chess.PAWN, chess.WHITE if flip else chess.BLACK))
        if cap_type == 1:
            board.set_piece_at(
                sq,
                chess.Piece(chess.QUEEN, chess.BLACK if flip else chess.WHITE))
...
def main(cmd):
    ...
    def first(input_data):
        ...
        occupency_policy_mask = tf.reshape(
            tf.tile(
                tf.reduce_max(input_data[:, 0:12, :, :], axis=1,
                              keepdims=True), [1, 128, 1, 1]), [-1, 128 * 64])
        illegal_filler = tf.zeros_like(policy) - 1.0e10
        policy = tf.where(tf.equal(occupency_policy_mask, 0), policy,
                          illegal_filler)
        ...
        return policy, moves, r50_est, sorted_high_policy, sorted_indicies[:,
                                                                           1]
    ...
    while True:
        ...
        if instruction == 'uci':
            ...
            print('Move type:', max_idx // 64, 'Starting Square:',
                  max_idx % 64)
            ...
            for i in range(len(root_node.children)):
                ...
                print('Child:', root_node.children[i].visits,
                      root_node.policy_index[i])

            reco_moves.append(updateBoardForIndex(board, state, max_idx, flip))
            input_data, flip = calculate_input('position fen ' + board.fen(),
                                               board, state)
            bestmove = '0000'
            print('Recomoves:', list(reversed(reco_moves)))
            instruction_override = instruction
            if board.board_fen(
            ) == "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR" and not flip:
                board.reset()
